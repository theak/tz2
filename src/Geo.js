...
function simplifySecondary(secondary_text) {
...
export default class Geo {  
  ...
  getCities(query, callback) {
    //Might wanna try getQueryPredictions instead...
  ...
  renderPrediction(prediction) {
    ...
    const cityStr = structuredFormat.main_text + ', ' + simplifySecondary(structuredFormat.secondary_text);
  ...
  getCity(callback) {
    fetchJsonp(GEOLOC_URL, {jsonpCallback: 'jsoncallback'})
        ...
        .then((json) => {
            ...
            const secondary_text = simplifySecondary(json.geoplugin_region + ', ' + json.geoplugin_countryName);
            callback(main_text + ', ' + secondary_text);
          })
        .catch((err) => {console.log('json parsing failed :(')});
